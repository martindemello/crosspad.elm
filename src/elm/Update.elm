...
read_file f model =
  let res = Reader.decode f
  in
      case res of
        Ok xw' -> {model | xw = xw', symmetry = GridLocked }
        _ -> model
...
update_model msg model =
  case msg of
    ...
    UploadFile -> model
    FileUploaded f -> read_file f model
    SaveFile -> model

update: Msg -> Model -> (Model, Cmd Msg)
update msg model =
  case msg of
    UploadFile -> (model, fileSelected "file-upload")
    _ -> (update_model msg model, Cmd.none)
